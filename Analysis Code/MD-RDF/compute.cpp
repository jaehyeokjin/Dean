...
int main(int argc, char *argv[]) {
    ...
    Species pivot_species = parse_species("M");
    Species other_species = parse_species("M");

    // System specs
    size_t n_sites = 400;
    double box_length = 30.0;
    double box_volume = box_length * box_length;
    double wcut = 0.8;

    // Histogram specs
    size_t n_histogram_bins = 751;
    double histogram_cutoff = 15.0;
    ...
    for (size_t i = 0; i < n_histogram_bins; ++i) {
        ...
        histogram_vols[i] = (4.0) * M_PI * pow(histogram_binwidth, 2.0) * ((double) (2 * i  + 1));
            ...
            if ((pivot_species != kN && type[i] == 1) || (pivot_species == kN && type[i] == 2)) {
                ...
                if (pivot_species == other_species) {
                    ...
                    for (size_t j = 0; j < n_sites; ++j) {
                        if (i != j && ((other_species != kN && type[j] == 1) || (other_species == kN && type[j] == 2)))  {
                            // Calculate the distance squared.
                            double dx = x[i] - x[j];
                            double dy = y[i] - y[j];
                            min_image(dx, box_length);
